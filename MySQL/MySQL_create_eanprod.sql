...
CREATE DATABASE eanprod
  CHARACTER SET utf8
  COLLATE utf8_unicode_ci;

## users permisions, you must run as root to get the user this permissions
##
GRANT ALL ON eanprod.* TO 'eanuser'@'%'
IDENTIFIED BY 'Passw@rd1';
GRANT ALL ON eanprod.* TO 'eanuser'@'localhost'
IDENTIFIED BY 'Passw@rd1';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, RELOAD, PROCESS, REFERENCES, INDEX, ALTER, SHOW DATABASES, CREATE TEMPORARY TABLES, LOCK TABLES, EXECUTE, REPLICATION SLAVE, REPLICATION CLIENT, CREATE VIEW, SHOW VIEW, CREATE ROUTINE, ALTER ROUTINE, CREATE USER, EVENT, TRIGGER, LOAD FROM S3 ON *.* TO eanuser@'%' IDENTIFIED BY 'Passw@rd1';
...
CREATE INDEX activeproperties_geoloc
  ON activepropertylist (Latitude, Longitude);
...
CREATE INDEX idx_propertyattributelink_reverse
  ON propertyattributelink (AttributeID, EANHotelID);
...
(
  ...
  CHARACTER SET utf8
  COLLATE utf8_unicode_ci;
CREATE INDEX idx_hotelimagelist_eanhotelid
  ON hotelimagelist (EANHotelID);
...
DELIMITER $$
CREATE PROCEDURE sp_hotels_from_point_restrict(IN lat  DOUBLE, lon DOUBLE, maxdist INT, country VARCHAR(200),
                                                  city VARCHAR(200))
  BEGIN
    SET @s = CONCAT(
        'SELECT EanHotelID,Name,Address1,Address2,City,StateProvince,PostalCode,Country,StarRating,LowRate,HighRate,Latitude,Longitude,',
...
CREATE PROCEDURE sp_airport_from_point(IN lat DOUBLE, lon DOUBLE, countrycode VARCHAR(2), maxrec INT)
  BEGIN
    SET @s = CONCAT('SELECT AirportID,AirportCode,AirportName,CountryCode,Latitude,Longitude,',
                    ' round( sqrt((POW(a.Latitude-', lat, ',2)*68.1*68.1)+(POW(a.Longitude-', lon,
                    ',2)*53.1* 53.1))) AS distance',
                    ' FROM airportcoordinateslist AS a ',
                    ' WHERE CountryCode=', '\'', countrycode, '\'',
                    ' AND AirportName NOT LIKE \'%Heliport%\' AND AirportName NOT LIKE \'%Train%\'',
...
CREATE INDEX log_activeproperties
  ON log_activeproperty_changes (TimeStamp, EANHotelID, FieldName);
...
CREATE PROCEDURE sp_log_createcopy()
  BEGIN
    ...
    INSERT oldactivepropertylist SELECT *
...
CREATE PROCEDURE sp_log_addedrecords()
  BEGIN
    ...
    SELECT @max_eanid := MAX(EANHotelID)
    ...
    INSERT INTO log_activeproperty_changes (EANHotelID, FieldName, FieldType, FieldValueOld, FieldValueNew)
      SELECT
        NOW.EANHotelID,
        'EANHotelID'         AS FieldName,
        'int'                AS FieldType,
        NULL                 AS FieldValueOld,
        'reactivated record' AS FieldValueNew
  ...
  BEGIN
    ...
    INSERT INTO log_activeproperty_changes (EANHotelID, FieldName, FieldType, FieldValueOld, FieldValueNew)
      SELECT
        ...
        'int'              AS FieldType,
        NULL               AS FieldValueOld,
        'stop-sell record' AS FieldValueNew
      FROM oldactivepropertylist AS OLD
...
CREATE PROCEDURE sp_log_changedrecords()
  BEGIN
    ...
    read_loop: LOOP
      FETCH cur
      INTO oEANHotelID, oName, oAddress1, oAddress2, oCity, oStateProvince, oPostalCode, oCountry,
